...
def restore_image_from_quadrants(quadrants):
    ...
    top_row = torch.cat((quadrants['1'], quadrants['2']), dim=2)  # Concatenate along width (W)
    bottom_row = torch.cat((quadrants['3'], quadrants['4']), dim=2)  # Concatenate along width (W)

    # Concatenate vertically (top_row and bottom_row)
    #TODO maybe vit need this
    full_image_tensor = torch.cat((top_row, bottom_row), dim=1)  # Concatenate along height (H)
    ...
    img = inv_transform(full_image_tensor)  # Assuming the tensor is normalized with (C, H, W)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--root_dir', type=str, required=True, help='root directory of the dataset folder')
    parser.add_argument('--ckp_path', type=str, required=True, help='checkpoint path')
    parser.add_argument('--save_dir', type=str, help='Directory to save test image results')
    parser.add_argument('--is_cropped', action='store_true', help='Indicate whether trained based on cropped image')
