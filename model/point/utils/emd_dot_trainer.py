...
class EMDTrainer(Trainer):
    def setup(self):
        ...
        # Initialize wandb
        wandb.init(
            project="berry-counting",  # Set your project name
            name=args.save_dir.split("/")[-1],  # Use save_dir as run name
            config=args,  # Log hyperparameters
        )
    ...
    def train_eopch(self, epoch=0):
        ...
        shape = (
            1,
            int(512 / self.args.downsample_ratio),
            int(512 / self.args.downsample_ratio),
        )
        ...
        print(
            "learning rate: {}, batch size: {}".format(
                self.optimizer.param_groups[0]["lr"], self.args.batch_size
            )
        )
        for step, (inputs, points, targets, st_sizes) in enumerate(
            ...
            shape = (
                inputs.shape[0],
                int(inputs.shape[2] / self.args.downsample_ratio),
            ...
            with torch.autograd.set_grad_enabled(True):
                ...
                cood_grid = grid(outputs.shape[2], outputs.shape[3], 1).unsqueeze(
                    0
                ) * self.args.downsample_ratio + (self.args.downsample_ratio / 2)
                cood_grid = cood_grid.type(torch.cuda.FloatTensor) / float(
                    self.args.crop_size
                )
                i = 0
                emd_loss = 0
                ...
                for p in points:
                    if len(p) < 1:
                        ...
                        PI = (
                            torch.exp(
                                (
                                    F.repeat(1, 1, C.shape[2])
                                    + G.permute(0, 2, 1).repeat(1, C.shape[1], 1)
                                    - C
                                ).detach()
                                / self.args.blur**self.args.p
                            )
                            * A
                            * gt.permute(0, 2, 1)
                        )
                        ...
                        if self.args.d_point == "l1":
                            point_loss += (
                                torch.sum(torch.abs(PI.sum(1).reshape(1, -1, 1) - gt))
                                / shape[0]
                            )
                        else:
                            point_loss += (
                                torch.sum((PI.sum(1).reshape(1, -1, 1) - gt) ** 2)
                                / shape[0]
                            )
                        if self.args.d_pixel == "l1":
                            pixel_loss += (
                                torch.sum(
                                    torch.abs(PI.sum(2).reshape(1, -1, 1).detach() - A)
                                )
                                / shape[0]
                            )
                        else:
                            pixel_loss += (
                                torch.sum(
                                    (PI.sum(2).reshape(1, -1, 1).detach() - A) ** 2
                                )
                                / shape[0]
                            )
                    i += 1
                ...
                loss = (
                    emd_loss
                    + self.args.tau * (pixel_loss + point_loss)
                    + self.blur * entropy
                )
        ...
        # After training epoch, log metrics to wandb
        wandb.log(
            {
                "epoch": self.epoch,
        ...
        if (mae) < (self.best_mae[stage]):
            ...
            logging.info(
                "{} save best mse {:.2f} mae {:.2f} model epoch {}".format(
                    stage, self.best_mse[stage], self.best_mae[stage], self.epoch
                )
            )
            torch.save(
                model_state_dic,
                os.path.join(self.save_dir, "best_{}.pth").format(stage),
            )
        # print log info
        logging.info(
            "Val: Best Epoch {} Val, MSE: {:.2f} MAE: {:.2f}, Cost {:.1f} sec".format(
                self.best_epoch["val"],
                self.best_mse["val"],
                self.best_mae["val"],
                time.time() - epoch_start,
        ...
        wandb.log(
            {
                f"{stage}/mae": mae,
                f"{stage}/rmse": mse,
                f"{stage}/best_mae": self.best_mae[stage],
                f"{stage}/best_rmse": self.best_mse[stage],
            }
        )
